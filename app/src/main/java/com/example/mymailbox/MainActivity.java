   ...
   // DatabaseHelper mydb;
    ...
    private class MakeRequestTask extends AsyncTask<Void, Void, List<String>> {
        ...
        private  List<String>  getDataFromApi() throws IOException {
            ...
            Message mess = mService.users().messages().get(user, response.getMessages().get(0).getId()).execute();
            MessagePart part = mess.getPayload();
            //Log.d("yun",StringUtils.newStringUtf8(Base64.decodeBase64(part.getBody().getData())));
            for (Message message : response.getMessages()) {
                Message readableMessage = mService.users().messages().get(user, message.getId()).execute();
                //  Log.d("information",readableMessage.getRaw());
                // Log.d("meid",message.getId()); id of every email
                // Log.d("body",readableMessage.getPayload().getBody().getData());
                if (readableMessage.getPayload() != null) {
                    //Log.d("infomation",readableMessage.getId()); 15d7a16932ec5a46 id for each mail
                    // Log.d("information",readableMessage.getInternalDate().toString()); 1500991854000 time to string
                    //Log.d("information",readableMessage.getThreadId()); 15d792798cf497ee true queue
                    for (MessagePartHeader header : readableMessage.getPayload().getHeaders()) {
                        // Log.d("header",header.getValue()); all value
                        //  Log.d("information",readableMessage.getPayload().setPartId("15d792798cf497ee").getBody().getData());

                        if (header.getName().compareToIgnoreCase("Subject") == 0) {
                            //  Log.d("header",header.getName()); //subject
                            messages.add(header.getValue());
                        }
                    }
                }
                i++;
        ...
        public void AddData(){
            button_add.setOnClickListener(
                    new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            boolean myinsert=mydb.Insert("hhhh");
                            if (myinsert==true)
